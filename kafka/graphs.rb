...
class RPlotter
  ...
  def initialize(title, y_label, event_file, metric_files, opts={})
    ...
    @max_value = (@metrics.values.map { |series| series.values.max}.max.to_f * 1.1).to_s
  end
  ...
  def plot!(svg_file_name)
    ...
    IO.popen("R --no-save","w") do |r|
      ...
      @metrics.each do |name, values|
        ...
        r.puts(%Q{#{method}(a_keys, a_vals, ylim=c(0,#{@max_value}), main="#{@title}", type="b",pch=3, xlab="Time (s)", ylab="#{@y_label}", col="#{color[name]}")})
